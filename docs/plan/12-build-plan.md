...
**2b in progress, 2026-08-12.** The 205-question set is assembled and gated:

- 165 new questions (q041–q205) across Buying, Stock, and cross-module seams,
  plus the frozen forty carried verbatim.
- `nakhoda/bench/questions.py` exports `Q`; `nakhoda/tests/test_questions.py`
  enforces contiguous ids, no duplicate text, frozen-text immutability, every
  gold query returns ≥1 row, `ordered=True` always has an ORDER BY, every trap
  is documented, module labelling is honest, and cross-module questions read
  ≥2 modules.
- The CLI learned `--set full`, which uses the new questions and the
  clean-room semantic context generated by `fixture.py`, grading against
  `/tmp/nakhoda-fixture/erp.duckdb`.
- The fixture was rebuilt against ERPNext v16.29.0 and byte-matched to the
  frozen semantic-bench database on the shared tables.

The full measurement is planned (2 arms × 2 targets × 3 tiers × 205 questions =
2,460 calls) and a 342-call smoke/partial run completed before the model account
hit a hard rate limit (`retry-after-ms ≈ 8,515,000`, i.e. ~2h22m). Partial cells
are encouraging but not publishable:

- A_raw / ops: 72/84 = 85.7% (CI [76.7, 91.6])
- A_raw / sql: 69/88 = 78.4% (CI [68.7, 85.7])
- B_semantic / ops: 77/81 = 95.1% (CI [88.0, 98.1])
- B_semantic / sql: 80/89 = 89.9% (CI [81.9, 94.6])
- McNemar ops A vs B: 78 pairs, discordant 1/8, p = 0.039

Resume once the limit resets:

```bash
cd /Users/mac/ERPNext/gardatest/apps/nakhoda
# generate remaining completions from /tmp/nakhoda-2b/raw.jsonl, then grade
python -m nakhoda.bench --set full --run /tmp/nakhoda-2b run --models replay
python -m nakhoda.bench --set full --run /tmp/nakhoda-2b grade
```

If the rate limit is structural, reduce to the gate-only run:

```bash
python -m nakhoda.bench --set full --run /tmp/nakhoda-2b-gate plan \
  --arms A_raw,B_semantic --targets sql,ops --tiers default
```
